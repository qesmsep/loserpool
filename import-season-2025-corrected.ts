...
async function importSeason2025Corrected() {
    ...
    try {
      ...
      for (const game of games) {
        try {
          ...
          // Insert new matchup with CORRECT season mapping
          const { error: insertError } = await supabase
            .from('matchups')
            .insert({
              week: weekInfo.week,
              season: weekInfo.season, // This is the key fix - using the correct season string
...
importSeason2025Corrected().catch(console.error)

...
export async function GET(request: NextRequest) {
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      console.log('🔍 Admin matchups API: Using bearer token authentication');
    ...
    if (!authenticatedUser) {
      console.log('🔍 Admin matchups API: Falling back to cookie authentication');
    ...
    if (!authenticatedUser) {
      console.log('❌ No authenticated user found for admin matchups API');
    ...
    console.log('✅ Admin matchups API: Success for user:', authenticatedUser.email);
    ...
    console.error('Admin matchups API error:', error);

...
export async function POST(request: NextRequest) {
  try {
    ...
    const testData = {
      ...
      firstName: 'Tim',
      lastName: 'Wirick',
      confirmationLink: 'https://loserpool.vercel.app/api/auth/confirm-email?email=tim@828.life&token=test-token-12345'
    }

    console.log('=== SENDING TEST SIGNUP EMAIL TO tim@828.life ===')
    console.log('Test Signup Data:', testData)

    // Send signup confirmation email with direct email call
    console.log('📧 Sending signup confirmation email...')
    const signupResult = await sendEmailDirect({
          ...
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <div style="padding: 32px;">
              ...
              <div style="text-align: center; margin-bottom: 24px;">
                <a href="${testData.confirmationLink}" style="display: inline-block; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 16px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; box-shadow: 0 4px 6px rgba(16, 185, 129, 0.3);">
    ...
    console.log('✅ Signup confirmation email sent with ID:', signupResult.id)

    console.log('=== TEST SIGNUP EMAIL COMPLETED ===')

    return NextResponse.json({ 
      success: true, 
      message: 'Test signup email sent successfully to tim@828.life',
      emailSent: `Signup Confirmation Email (ID: ${signupResult.id})`,
      confirmationLink: testData.confirmationLink
    })

  } catch (error) {
    console.error('Error sending test signup email:', error)
    return NextResponse.json(
      { error: 'Failed to send test signup email', details: error },
      { status: 500 }
    )
  }
}
